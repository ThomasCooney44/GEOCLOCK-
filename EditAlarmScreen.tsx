...
import { LocationCoords, GeoAlarm } from '../types';
...
export const EditAlarmScreen: React.FC = () => {
  ...
  const loadAlarm = async () => {
    try {
      const loadedAlarm = await GeoAlarmStorage.getAlarm(alarmId);
      if (loadedAlarm) {
        setAlarm(loadedAlarm);
        setName(loadedAlarm.name);
        setLocation({
          latitude: loadedAlarm.latitude,
          longitude: loadedAlarm.longitude,
        });
        setAddress(loadedAlarm.address || '');
        setRadiusMeters(loadedAlarm.radiusMeters);
        setEnabled(loadedAlarm.enabled);
        setDisableAfterTrigger(loadedAlarm.disableAfterTrigger);
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      const updatedAlarm = await GeoAlarmStorage.updateAlarm(alarmId, {
        name: name.trim(),
      ...
      if (updatedAlarm) {
        // Update geofence
        GeofenceService.removeGeofence(alarmId);
        GeofenceService.addGeofence(updatedAlarm);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await GeoAlarmStorage.deleteAlarm(alarmId);
              GeofenceService.removeGeofence(alarmId);
              navigation.goBack();
            } catch (error) {
